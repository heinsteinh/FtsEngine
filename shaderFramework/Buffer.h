...
public:
	...
	*/
	GLvoid *mapRange(GLenum _target, GLintptr _offset, GLsizeiptr _length, GLbitfield _access) {
		bind(_target);
		GLvoid *ret = NULL; //glMapBufferRange(_target, _offset, _length, _access);
		return ret;
	}
	...
	*/
	...
	GLvoid *map(GLenum _target, GLenum _access) {
		bind(_target);
		GLvoid *ret = NULL; //glMapBuffer(_target, _access);
		return ret;
	}
	...
	GLboolean unmap(GLenum _target) {
		bind(_target);
		GLboolean ret = NULL; //glUnmapBuffer(_target);
		return ret;
	}
...
protected:
	...
	GLint mSize; // as this might get queried often (e.g. ArrayBuffer) we will explicitly mirror it in RAM)
};
