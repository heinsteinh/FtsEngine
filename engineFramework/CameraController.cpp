...
#undef far
		...
		void CameraController::updateCamera() {
			...
			if (!mUseSplines) {
				...
				if (mSpline != NULL) {
					static float t = 0;
					t += /* time * */ STEP_DISTANCE / 10.0f;
					if (t >= 1.0)
						t = 0.0; //actually we are finished!!
					mCameraPosition = mSpline->getInterpolatedSplinePoint(t);
				}
			}
		}

		//void CameraController::setSplineT
		...
		void CameraController::setSpline(const CRSpline &spline) {
			if (mSpline)
				delete mSpline;

			mSpline = new CRSpline(spline);
		}
		...
		glm::mat4 CameraController::getModelViewMat(void) {
			...
			//if splines are used, involve aim points to look at from control points
		}

		glm::vec3 CameraController::getAimVec(void) {
			// return the modelview matrix describing the position and orientation of the camera //
			// compute a simple lookAt position relative to the camera's position //
			glm::vec3 lookDir(sin(mTheta) * cos(mPhi), sin(mPhi), cos(mTheta) * cos(mPhi));
			return mCameraPosition - lookDir;

			//if splines are used, involve aim points to look at from control points
		}

		glm::vec3 CameraController::getLookDir(void) {
			// return the modelview matrix describing the position and orientation of the camera //
			// compute a simple lookAt position relative to the camera's position //
			glm::vec3 lookDir(sin(mTheta) * cos(mPhi), sin(mPhi), cos(mTheta) * cos(mPhi));
			return lookDir;
		...
		glm::vec3 CameraController::getInvertLookDir(void) {
			glm::vec3 lookDir(sin(-mTheta) * cos(-mPhi), sin(-mPhi), cos(-mTheta) * cos(-mPhi));
			return lookDir;
		}


		glm::vec3 CameraController::getInvertAimVec(void) {
			glm::vec3 lookDir = getInvertLookDir();
			return mCameraPosition - lookDir;
		}
		...
		void CameraController::resetOrientation(float theta, float phi, float dist) {
			...
			mCameraPosition = glm::vec3(sin(mTheta) * cos(mPhi) * dist, sin(mPhi) * dist, cos(mTheta) * cos(mPhi) * dist);
