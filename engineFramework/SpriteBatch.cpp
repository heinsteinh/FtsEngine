...
namespace rendering
{
	...
	SpriteBatch::SpriteBatch(unsigned int size)	: currentTexture(NULL), renderCalls(0), bufIndex(0), indIndex(0), drawing(false)
	{
		...
		unsigned short indicesSize = 6 * size / (VERTEX_SIZE * 4);
		indices = new GLushort[indicesSize];
		...
		glm::vec2 scale, glm::vec4 tint, float rotation)
	{
		...
		transform = glm::scale(transform,glm::vec3(width * scale.x, height * scale.y, 1.0f));
