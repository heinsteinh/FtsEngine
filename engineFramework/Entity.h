...
#define ENTITYX_MAX_COMPONENTS 1024*2
...
/*usage
	https://github.com/alecthomas/entityx/blob/master/entityx/Entity_test.cc
*/
...
namespace entityx
{
	...
	*/
	struct EntityCreatedEvent //: public core::EventHandler<EntityCreatedEvent>
	{
	...
	*/
	struct EntityDestroyedEvent //: public core::EventHandler<EntityDestroyedEvent> 
	{
	...
	*/
	template <typename C>
	struct ComponentAddedEvent //: public core::EventHandler<ComponentAddedEvent<C>>
	{
		ComponentAddedEvent(Entity entity, ComponentHandle<C> component) :
		entity(entity), component(component) 
		{			
			echan.add<ComponentAddedEvent<C>>(*this);
		}

		~ComponentAddedEvent()
		{
			echan.remove<ComponentAddedEvent<C>>(*this);
		}


		void handle(const ComponentAddedEvent<C>&v)
		{
			std::cout << "Add Comp ID: " << " Entity with ID: " << v.entity.id().id() << std::endl;
		}
	...
	/*
		struct ComponentAddedEventReceiver : public
		core::EventHandler<entityx::ComponentAddedEvent<EntityTest::Position>>,
		core::EventHandler<entityx::ComponentAddedEvent<EntityTest::Direction>>
	{
		void handle(const entityx::ComponentAddedEvent<EntityTest::Position> &event)
		{
		...
		void handle(const entityx::ComponentAddedEvent<EntityTest::Direction> &event)
		{
			auto p = event.component;
	...
	emf.ev.add<EntityTest::ComponentAddedEvent<EntityTest::Position>>(receiver);
	emf.ev.add<EntityTest::ComponentAddedEvent<EntityTest::Direction>>(receiver);
	*/
	...
	*/
	template <typename C>
	struct ComponentRemovedEvent //: public core::EventHandler<ComponentRemovedEvent<C>> 
	{
		ComponentRemovedEvent(Entity entity, ComponentHandle<C> component) :
		entity(entity), component(component) 
		{
			core::EventChannel echan;
			echan.add<ComponentRemovedEvent<C>>(*this);
		}
		~ComponentRemovedEvent()
		{
			echan.remove<ComponentRemovedEvent<C>>(*this);
		}


		void handle(const ComponentRemovedEvent<C>&v)
		{
			std::cout << "ComponentAddedEventhandler Entity with ID: " << v.entity.id().id() << std::endl;
		}
	...
	*/
	...
	public:
		...
		void handle(const EntityDestroyedEvent& v)
		{
			std::cout << "Destroy Entity with ID: " << v.entity.id().id() << std::endl;
		}
		
		void handle(const EntityCreatedEvent& v)
		{
			std::cout << "Newly create Entity with ID: " << v.entity.id().id() << std::endl;
		}
		...
		*/
		Entity create() 
		{
			...
			event_manager_.broadcast(EntityCreatedEvent(entity));
		...
		*/
		void destroy(Entity::Id entity)
		{
			...
			event_manager_.broadcast(EntityDestroyedEvent(Entity(this, entity)));
		...
		*/
		...
		ComponentHandle<C> assign(Entity::Id id, Args && ... args)
		{
			...
			core::EventChannel event_manager_;
			event_manager_.broadcast(ComponentAddedEvent<C>(Entity(this, id), component));
			return component;
		}
	...
	private:
		...
		C *get_component_ptr(Entity::Id id) {
			assert(valid(id));
			BasePool *pool = component_pools_[(unsigned int)C::family()];
			assert(pool);
		...
		Pool<T> *accomodate_component() {
			...
			if (!component_pools_[(unsigned int)family]) {
				Pool<T> *pool = new Pool<T>();
				pool->expand(index_counter_);
				component_pools_[(unsigned int)family] = pool;
			}
			return static_cast<Pool<T>*>(component_pools_[(unsigned int)family]);
		}
