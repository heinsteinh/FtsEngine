...
namespace ANAS {
	...
	template<> class Uniform<glm::mat2, 2> : public UniformBase{
	public:

		void SetValue(const glm::mat2& mat, bool transpose = false, int count = 1){
			glUniformMatrix2fv(UniformLocation, count, transpose, glm::value_ptr(mat));
		}
	};


	template<> class Uniform<glm::mat3, 3> : public UniformBase{
	public:
		
		void SetValue(const glm::mat3& mat,bool transpose = false, int count = 1){
			glUniformMatrix3fv(UniformLocation, count, transpose, glm::value_ptr(mat));
		}
	};


	template<> class Uniform<glm::mat4, 4> : public UniformBase{
	public:

		void SetValue(const glm::mat4& mat, bool transpose = false, int count = 1){
			glUniformMatrix4fv(UniformLocation, count, transpose, glm::value_ptr(mat));
		}
	};
...
namespace zephyr {
	namespace gfx {
		...
		typedef UniformArray<GLfloat, decltype(&glUniform1iv), &glUniform1iv> uniform1uiv;
		typedef UniformArray<GLfloat, decltype(&glUniform2iv), &glUniform2iv> uniform2uiv;
		typedef UniformArray<GLfloat, decltype(&glUniform3iv), &glUniform3iv> uniform3uiv;
		typedef UniformArray<GLfloat, decltype(&glUniform4iv), &glUniform4iv> uniform4uiv;
