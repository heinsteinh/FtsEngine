...
// TODO: Size should be a vec2 but I don't feel like changing the sizing logic
Mesh * GeometryCreator::CreateQuad(glm::vec3 const & Size)
{
	std::vector<float> Positions, Normals;
	std::vector<unsigned short> Indices;

	static float const QuadPositions[] =
	{
		-0.5, -0.5, 0,
		-0.5, 0.5, 0,
		0.5, 0.5, 0,
	...
	Positions = std::vector<float>(QuadPositions, QuadPositions + 4 * 3);
	int i = 0;
	for (std::vector<float>::iterator it = Positions.begin(); it != Positions.end(); ++it, ++i)
		* it *= Size[i %= 3];

	static float const QuadNormals[] =
	{
		0, 0, 1,
		0, 0, 1,
		0, 0, 1,
	...
	Normals = std::vector<float>(QuadNormals, QuadNormals + 4 * 3);

	static unsigned short const QuadIndices[] =
	{
		0, 1, 2,
		2, 3, 0
	};
	Indices = std::vector<unsigned short>(QuadIndices, QuadIndices + 2 * 3);
	...
	unsigned int const stacks)
{
	...
	for (unsigned int i = 1; i <= slices; ++i)
	{
		...
		Positions.push_back(glm::cos<float>(Angle)*baseRadius);
		Positions.push_back(glm::sin<float>(Angle)*baseRadius);
	...
	{
		float const Angle = (float)i * 2.f * 3.14159f / slices;
		Positions.push_back(glm::cos<float>(Angle)*topRadius);
		Positions.push_back(glm::sin<float>(Angle)*topRadius);
	...
	for (unsigned int j = 0; j <= stacks; ++j)
	{
		...
		for (unsigned int k = 0; k <= slices; ++k)
		{
			...
			Positions.push_back(glm::cos<float>(Angle)*Radius);
			Positions.push_back(glm::sin<float>(Angle)*Radius);
			Positions.push_back(Interpolation * height);
			Normals.push_back(glm::cos<float>(Angle));
			Normals.push_back(glm::sin<float>(Angle));
	...
	unsigned int const stacks)
{
	...
	for (unsigned int i = 1; i <= slices; ++i)
	{
		...
		Positions.push_back(glm::cos<float>(Angle)*innerRadius);
		Positions.push_back(glm::sin<float>(Angle)*innerRadius);
		Positions.push_back(0.f);
		Positions.push_back(glm::cos<float>(Angle)*outerRadius);
		Positions.push_back(glm::sin<float>(Angle)*outerRadius);
		Positions.push_back(0.f);
		Normals.push_back(0.f); Normals.push_back(0.f); Normals.push_back(-1.f);
		Normals.push_back(0.f); Normals.push_back(0.f); Normals.push_back(-1.f);
	...
	for (unsigned int j = 0; j <= stacks; ++j)
	{
		...
		for (unsigned int k = 0; k <= slices; ++k)
		{
			...
			Positions.push_back(glm::sin<float>(Angle)*outerRadius);
			Positions.push_back(Interpolation * height);
			Normals.push_back(glm::cos<float>(Angle)); Normals.push_back(glm::sin<float>(Angle)); Normals.push_back(0.f);
		}
...
Mesh * GeometryCreator::CreateSphere(glm::vec3 const & Radii, unsigned int const Slices, unsigned int const Stacks)
{
	...
	for (unsigned int i = 1; i <= Stacks; ++i)
	{
		...
		for (unsigned int j = 0; j <= Slices; ++j)
		{
			...
			glm::vec3 Radial = glm::vec3(
				glm::cos<float>(AngleH)*glm::sin<float>(AngleV),
				glm::cos<float>(AngleV),
				glm::sin<float>(AngleH)*glm::sin<float>(AngleV));
	...
	unsigned int const rings)
{
	...
	for (unsigned int j = 0; j <= rings; ++j)
	{
		...
		for (unsigned int k = 0; k <= sides; ++k)
		{
			...
			float RadialExtent = glm::sin<float>(RingsAngle) * Radius + (innerRadius + outerRadius) / 2.f;
			Positions.push_back(glm::cos<float>(RadialAngle)*RadialExtent);
			Positions.push_back(glm::sin<float>(RadialAngle)*RadialExtent);
			Positions.push_back(glm::cos<float>(RingsAngle)*Radius);
			Normals.push_back(glm::sin<float>(RingsAngle)*glm::cos<float>(RadialAngle));
			Normals.push_back(glm::sin<float>(RingsAngle)*glm::sin<float>(RadialAngle));
			Normals.push_back(glm::cos<float>(RingsAngle));
		}
