...
namespace zephyr {
	namespace gfx {
		...
		Renderer::Renderer(zephyr::resources::ResourceSystem& res) {
			/*glewInit();
		...
		public:
			...
			TextureBinder(ProgramPtr program): nextFreeUnit_(0), program_(std::move(program)){ }
		...
		void Renderer::setUniformsForCurrentProgram() {
			for (const auto& needed : currentProgram_->uniforms()) {
				...
				if (Uniform* value = uniforms_.get(name)) {
					value->set(needed.second->location);
				}
			}
		}
		...
		void Renderer::setProgram(const ProgramPtr& program) {
			if (currentProgram_ != program) {
				...
				{
					for (const auto& blocks : currentProgram_->uniforms()) {
						const std::string& name = blocks.first;
						GLuint index = blocks.second->location;
