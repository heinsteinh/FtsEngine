	...
	namespace utils {
		...
		static void dump_shader_info(GLint program) {
			...
			for (GLint uniform = 0; uniform < active_uniforms; ++uniform) {
				...
				glGetActiveUniform(program, uniform, uniform_max_length, &length, &size, &type, &name[0]);
				name.resize(length);
				...
				case GL_FLOAT: {
								   ...
								   glGetUniformfv(program, location, &felems.front());
								   std::cerr << " Elements: [ ";
				...
				case GL_INT: {
								 ...
								 glGetUniformiv(program, location, &ielems.front());
								 std::cerr << " Elements: [ ";
